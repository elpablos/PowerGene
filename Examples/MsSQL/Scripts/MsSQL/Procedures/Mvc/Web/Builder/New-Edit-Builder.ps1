...
Add-Line('            var result = _{0}Service.Detail(new Detail{0}InputModel' -f  $data.Metadata.Name, $data.Metadata.Prefix)
Add-Line('            {')
...
{  
    ...
    {
        Add-Line('            data.{0} = result.Data.{1};' -f ($column.Value.Name, 'Id')[$column.Value.Name -eq 'ID'], $column.Value.Name)
    }
}
